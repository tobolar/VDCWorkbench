...
model ControlAllocationRL
  ...
  parameter Modelica.Units.SI.Angle maxRL_steering = Modelica.Constants.pi
    "Saturation limit for RL steering correction [rad]";
  parameter Modelica.Units.SI.Torque maxRL_torque = 1000 "Saturation limit for RL torque correction [Nm]";
  ...
  Modelica.Blocks.Interfaces.RealOutput v_path
    "Velocity of path at current arc length value" annotation (Placement(
        transformation(
          ...
          rotation=180,
          origin={-10,-70})));
  Modelica.Blocks.Interfaces.RealOutput beta_geo "Desired side slip angle of geo pfc"
    annotation (Placement(transformation(
          extent={{0,-60},{-20,-40}}), iconTransformation(extent={{-52,62},{-30,84}})));
equation
  ...
  delta = baseline_delta
        + min(maxRL_steering, max(-maxRL_steering, addRL_FrontSteering_in));

  baseline_delta = atan2(vy_sens, vx_sens);

  // Total motortorque speed controller + limited RL correction
  baseline_torque = min(1.0 * torque_max, max(torque_min,
             Kspeedctrl * (v_path - vveh)));

  torque = min(1.0 * torque_max, max(torque_min,
        Kspeedctrl * (v_path - vveh)
      + min(maxRL_torque, max(-maxRL_torque, addRL_TotalTorque_in))));
