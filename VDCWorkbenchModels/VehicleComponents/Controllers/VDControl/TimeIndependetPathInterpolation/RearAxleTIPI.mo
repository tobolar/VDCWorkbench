...
model RearAxleTIPI "Rear axle time-independent path interpolation"
  ...
  parameter Modelica.Units.SI.Length lr = 0.1805 "Distance of CoG to rear axle";
  ...
  Real lambda[5];
  Real sI_rear[2] "Rear axle position in inertial frame";
...
public
  ...
  Modelica.Blocks.Tables.CombiTable1Ds combiTablePath_ff(
    smoothness=combiTablePath.smoothness,
    columns=combiTablePath.columns,
    tableOnFile=combiTablePath.tableOnFile,
    fileName=filePath,
    tableName=pathName,
    table=combiTablePath.table)                                                                                                                                                                                                         annotation (Placement(transformation(extent={{40,70},{60,90}})));
  Modelica.Blocks.Math.Add add
    annotation (Placement(transformation(extent={{-30,90},{-10,70}})));
  Utilities.Blocks.Modulo modulo1(k=maxArcLength) annotation (Placement(transformation(extent={{0,70},{20,90}})));
  Modelica.Blocks.Sources.RealExpression realExpression_sff(y=s_ff)
...
protected
  ...
  Modelica.Blocks.Interfaces.RealOutput vveh "Absolute vehicle speed" annotation (Placement(transformation(extent={{-10,-10},{10,10}}, rotation=180, origin={-40,-70})));
  Modelica.Blocks.Interfaces.RealOutput kappa_ff "Absolute vehicle speed"
...
equation
  ...
  // calc center of rear axle
  sI_rear[1] = sI_C[1] - lr * cos(sI_C[3]);
  sI_rear[2] = sI_C[2] - lr * sin(sI_C[3]);

  e = sI_rear - lambda[1:2]; // Calculate distance error in inertial frame I
  ...
  connect(vveh, chassisBus.longitudinalVelocity) annotation (Line(points={{-40,-70},{12,-70},{12,-80},{20,-80}},
        color={0,0,127}),
      Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
  connect(kappa_ff, motionDemandBus.kappa_ff) annotation (Line(points={{10,-10},{80,-10},{80,0}},
        color={0,0,127}),
      Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
  connect(sampler.y, add.u1) annotation (Line(points={{21,40},{30,40},{30,60},{-40,60},{-40,74},{-32,74}},
        color={0,0,127}));
  connect(modulo1.y, combiTablePath_ff.u) annotation (Line(points={{21,80},{38,80}}, color={0,0,127}));
  connect(add.y, modulo1.u)
    annotation (Line(points={{-9,80},{-2,80}},  color={0,0,127}));
  connect(realExpression_sff.y, add.u2) annotation (Line(points={{-79,80},{-50,80},{-50,86},{-32,86}},
        color={0,0,127}));
  connect(combiTablePath_ff.y[5], motionDemandBus.kappa_ff) annotation (Line(points={{61,80},{84,80},{84,0},{80,0}}, color={0,0,127}),
      Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
